...
def log_datafördelning(data, theta,alpha,beta):
    ...
    for x in data:
        if x==0.0:
            ...
            DF+=np.log(theta*beta_pdf(x))
    return DF

def log_prior(alpha, beta):
    return np.log(1**3) - loggamma(3) + (3 - 1)*np.log(alpha + beta + 1) - 1*(alpha + beta + 1) - np.log(alpha + beta) * 1
...
def log_posterior(data, theta, alpha, beta):
    ...
    return log_prior(alpha,beta) * log_datafördelning(data, theta, alpha, beta)

def make_contour_plot():
    alpha_grid = np.linspace(0.1, 10, 100)
    beta_grid = np.linspace(0.1, 10, 100)

    log_posterior_grid = [[log_posterior(data, theta, alpha, beta)
