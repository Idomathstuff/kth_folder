...
kan beskriva i termer av stokatiska variabler. Låt $T_i$ vara total betyg för deltagare $i$.
Låt $Z_{i,\text{först}}=\max(X_{i1},X_{i2},X_{i3},X_{i4})$ och 
\\$Z_{i,\text{andra}}=\max(\min(X_1, X_2), \min(X_1, X_3), \min(X_1, X_4), \min(X_2, X_3), \min(X_2, X_4), \min(X_3, X_4))$.
\\Vi vill simulera total betyget för varje deltagare $i$ som $T_i=Z_{i,\text{först}}+Z_{i,\text{andra}}+\max(Y_{i1},Y_{i2})$
